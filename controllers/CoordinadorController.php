...
class CoordinadorController extends BaseController {
    ...
    public function uploadExcel() {
        // OPTIMIZACIÓN: Aumentar tiempo de ejecución para archivos grandes
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['archivo_excel'])) {
            ...
            while (($data = fgetcsv($handle, 0, $delimiter)) !== false) {
                ...
                try {
                    ...
                    // CORRECCIÓN: Usar celular2 como respaldo si telefono está vacío o es '0'
                    ...
                    if (empty($cedula) || empty($telefono) || $telefono === '0') {
                        $errores++;
                        error_log("Error en fila {$rowNumber}: Datos obligatorios vacíos - Cédula: '{$cedula}', Teléfono: '{$telefono}', Celular: '{$celular}'");
                        continue;
                    }
        ...
        if (($handle = fopen($archivo_path, "r")) !== FALSE) {
            ...
            while (($data = fgetcsv($handle, 1000, $delimitador)) !== FALSE) {
                if (count($data) >= 2) { // Mínimo cedula y teléfono
                    ...
                    // CORRECCIÓN: Usar celular2 como respaldo si telefono está vacío o es '0'
                    if (empty($cliente['telefono']) || $cliente['telefono'] === '0') {
                        if (!empty($cliente['celular2'])) {
                            $cliente['telefono'] = $cliente['celular2'];
                        }
                    }
                    
                    // Solo agregar si tiene datos mínimos (cedula y telefono válido)
                    if (!empty($cliente['cedula']) && !empty($cliente['telefono']) && $cliente['telefono'] !== '0') {
                        $clientes[] = $cliente;
                    }
                }
                $linea++;
    ...
    private function procesarClientesCSV($clientes, $cargaId, $coordinadorId) {
        // OPTIMIZACIÓN: Aumentar tiempo de ejecución y memoria para procesos grandes
        set_time_limit(600); // 10 minutos
        ini_set('memory_limit', '512M');
        ...
        // OPTIMIZACIÓN: Usar transacción única para todo el proceso
        ...
        try {
            ...
            // OPTIMIZACIÓN: Procesar en lotes de 100 para commits intermedios
            $batchSize = 100;
            $processed = 0;
            ...
            foreach ($datosAgrupados as $cedula => $grupo) {
                try {
                    ...
                    if ($clienteExistente) {
                        ...
                        if (!$yaEnCarga) {
                            // OPTIMIZACIÓN: No usar transacción individual (ya estamos en una transacción)
                            $this->clienteModel->agregarClienteACargaConActualizacion($cargaId, $clienteExistente['id'], $grupo['info_cliente'], false);
                        }
                    ...
                    // OPTIMIZACIÓN: Commit intermedio cada 100 clientes para evitar timeout
                    $processed++;
                    if ($processed % $batchSize === 0) {
                        $this->pdo->commit();
                        $this->pdo->beginTransaction();
                        error_log("Procesados $processed de " . count($datosAgrupados) . " clientes únicos");
                    }
            ...
            $this->pdo->rollBack();
            error_log("Error crítico en procesarClientesCSV: " . $e->getMessage());
            throw $e;
        }
     ...
     * Muestra TODOS los clientes de la carga, no solo los no asignados
     */
    public function verClientes() {
        ...
        // Obtener TODOS los clientes de la carga (asignados y no asignados)
        $clientes = $this->clienteModel->getClientsByCargaIdAndCoordinador($carga_id, $coordinador_id);
        
        // Obtener el total real de clientes de la base de datos
        $total_clientes = $this->clienteModel->getTotalClientsByCargaIdAndCoordinador($carga_id, $coordinador_id);
        
        // Enriquecer los clientes con información de asignación
        // Usar JOIN para obtener asignaciones de forma más eficiente
        if (!empty($clientes)) {
            $clienteIds = array_column($clientes, 'id');
            $placeholders = implode(',', array_fill(0, count($clienteIds), '?'));
            
            // Obtener todas las asignaciones activas de una vez
            $sql = "SELECT ac.cliente_id, ac.asesor_id, ac.estado 
                    FROM asignaciones_clientes ac 
                    WHERE ac.cliente_id IN ($placeholders) 
                    AND ac.estado = 'asignado'";
            $stmt = $this->pdo->prepare($sql);
            $stmt->execute($clienteIds);
            $asignaciones = [];
            while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                $asignaciones[$row['cliente_id']] = $row;
            }
            
            // Agregar información de asignación a cada cliente
            foreach ($clientes as &$cliente) {
                if (isset($asignaciones[$cliente['id']])) {
                    $cliente['asesor_id'] = $asignaciones[$cliente['id']]['asesor_id'];
                    $cliente['estado_asignacion'] = 'Asignado';
                } else {
                    $cliente['asesor_id'] = null;
                    $cliente['estado_asignacion'] = 'Pendiente';
                }
            }
            unset($cliente); // Limpiar referencia
        }
        
        // Para la vista inicial, mostrar todos los clientes (sin límite)
        // Si hay más de 1000 clientes, se recomienda usar la búsqueda
        $clientes_vista = $clientes;
    ...
    public function gestionarEstadoBases() {
        ...
        // Pasar el PDO a la vista para evitar conexiones duplicadas
        $pdo = $this->pdo;
