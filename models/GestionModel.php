...
class GestionModel {
    ...
    public function createGestion($asignacionId, $tipo, $comentarios, $resultado = null, $monto = null, $duracion = null, $edad = null, $numPersonas = null, $valorCotizacion = null, $whatsappEnviado = null, $formaContacto = 'llamada', $obligacionId = null, $productoGestionado = null, $montoObligacion = null, $numeroObligacion = null, $noCuotas = null, $fechaPago = null, $valorCuota = null, $numeroCuota = null, $valorAcuerdo = null, $telefonoContacto = null) {
        try {
            ...
            return $this->crearGestion($gestionData);
            
        } catch (Exception $e) {
            error_log("Error en createGestion (legacy): " . $e->getMessage());
    ...
    public function crearGestion($gestionData) {
        try {
            ...
            try {
                ...
                error_log("Advertencia: Error al registrar actividad automática (no crítico): " . $e->getMessage());
                error_log("Stack trace: " . $e->getTraceAsString());
                // Continuar sin lanzar excepción para no afectar la gestión principal
            }
     ...
     * MEJORADO: Manejo robusto de errores y límite de reintentos
     */
    private static $actividadFailCount = 0;
    private static $actividadFailThreshold = 10;
    private static $actividadLastFailTime = 0;
    
    private function registrarActividadAutomatica($historialGestionId, $gestionData) {
        // Si ha fallado muchas veces recientemente, deshabilitar temporalmente
        $now = time();
        if (self::$actividadFailCount >= self::$actividadFailThreshold) {
            // Si pasaron más de 5 minutos desde el último fallo, resetear contador
            if (($now - self::$actividadLastFailTime) > 300) {
                self::$actividadFailCount = 0;
                error_log("Reseteando contador de fallos de registrarActividadAutomatica");
            } else {
                // Aún en período de bloqueo, saltar registro
                error_log("registrarActividadAutomatica deshabilitado temporalmente (demasiados fallos)");
                return false;
            }
        }
        ...
        try {
            ...
            // Obtener información del cliente y asesor con timeout
            ...
            // Establecer timeout para esta consulta
            ...
            // Registrar la gestión del producto (con manejo de errores)
            try {
                ...
                if ($actividadId) {
                    ...
                    error_log("Error al registrar actividad (retornó false)");
                    self::$actividadFailCount++;
                    self::$actividadLastFailTime = $now;
                    return false;
                }
            } catch (Exception $e) {
                error_log("Error al registrar gestión de producto: " . $e->getMessage());
                self::$actividadFailCount++;
                self::$actividadLastFailTime = $now;
                // Continuar con el log del sistema aunque falle el registro de actividad
            }
            
            // Registrar log del sistema (opcional, no crítico)
            try {
                ...
                // El log del sistema no es crítico, solo loggear el error
                error_log("Error al registrar log del sistema (no crítico): " . $e->getMessage());
            }
            
            // Si llegamos aquí, todo funcionó correctamente
            self::$actividadFailCount = 0; // Resetear contador de fallos
            return true;
            
        } catch (PDOException $e) {
            error_log("Error PDO en registrarActividadAutomatica: " . $e->getMessage());
            error_log("SQL State: " . $e->getCode());
            self::$actividadFailCount++;
            self::$actividadLastFailTime = time();
            return false;
        } catch (Exception $e) {
